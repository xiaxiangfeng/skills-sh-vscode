...
export class MarketplaceParser {
  parseResponse(response: string, category: MarketplaceCategory): MarketplaceParseResult {
  ...
  parseSkillsFromJson(data: any): MarketplaceSkill[] {
    ...
    for (const item of items) {
      ...
      const updatedAt = this.parseUpdatedAt(
        item.updatedAt ||
          item.updated_at ||
          item.lastUpdated ||
          item.last_updated ||
  ...
  private parseUpdatedAt(value: unknown): number | undefined {
    if (typeof value === 'number' && Number.isFinite(value)) {
